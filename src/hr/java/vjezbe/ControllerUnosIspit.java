...
//	List<Ispit> listaIspita = metodeDatoteka.fillIspitFromFile(listaIspitaString, listaPredmeta, listaStudenta);
	...
	List<Predmet> listaPredmtSQL = bazaPodataka.getAllPred();
	List<Student> listStudenSQL = bazaPodataka.getAllStud();

	@Override
	public void initialize(URL location, ResourceBundle resources) {

		choiceBoxPredmet.getItems().addAll(listaPredmtSQL);
		choiceBoxStudent.getItems().addAll(listStudenSQL);

		choiceBoxPredmet.setValue(listaPredmtSQL.get(0));
		choiceBoxStudent.setValue(listStudenSQL.get(0));

	}

	public void entryIspit() {
		
		
		Ispit newIspit = null;
		OptionalLong maxId = listaIspitaSQL.stream().mapToLong(ispit -> ispit.getId()).max();
...
//		}
	...
	//		listaIspita.add(newIspit);
			listaIspitaSQL.add(newIspit);
			bazaPodataka.insertNewIspit(newIspit);
